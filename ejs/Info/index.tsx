...
const OrgInfo = () => {
  ...
  const getCarbonDetailFn = async () => {
    await GETa31af0d96d2a2d7ad3b3c82b167d48ac({}).then(({ data }) => {
      setemissionStandardEdit(Number(data?.data?.emissionStandardEdit) || 0);
    });
  };
  // 获取排放量数据
  ...
  useEffect(() => {
    if (id && PageTypeInfo.add !== pageTypeInfo) {
      GETba76fb8b18c3107ccadf772bdb13f241({ id: +id }).then(({ data }) => {
        if (data.code === 200) {
            ...
            onClick: async () => {
              return form.submit(async (values) => {
                ...
                if (
                  ...
                  await POSTbf1edfbdd5804ea69c4c785a021ed7c2({
                    req: {
                      ...newValue,
                    },
                  }).then(({ data }) => {
